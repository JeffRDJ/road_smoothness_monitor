...
class UDPBufferedSender:
    def __init__(self, config_path="config/config.json"):
        ...
        try:
            ...
            print("[Init] UDP发送端就绪，绑定本地地址: {}".format(self.sock.getsockname()))
#			print("[Init] UDP发送端就绪，绑定本地地址: {self.sock.getsockname()}")
            print("[Init] 目标发送地址:{}".format(self.target_addr))
        except Exception as e:
            print("[Error] 绑定失败:{}".format(e))

        # 5. 启动后台消费者线程
        self.running = True
        #self.worker_thread = threading.Thread(target=self._send_loop, daemon=True)
        self.worker_thread = threading.Thread(target=self._send_loop)
        self.worker_thread.setDaemon(True) # Python 2 的写法
    ...
    def _load_config(self, path):
        if not os.path.exists(path):
            return {
                "target_ip": "255.255.255.255", "target_port": 55555,
                "local_ip": "192.168.1.102", "local_port": 55555,
